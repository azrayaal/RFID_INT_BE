...
export const getLoading = async (
  ...
  try {
    const query = `
        ...
        WHERE loading.location_id = ?
`;

    const [rows] = await pool.query<RowDataPacket[]>(query, [location]);
...
export const getDetailLoading = async (
  req: Request,
  res: Response
): Promise<void> => {
  try {
...
export const deleteLoading = async (
  req: Request,
  res: Response
): Promise<void> => {
  try {
...
export const editLoading = async (
  req: Request,
  res: Response
): Promise<void> => {
  const { id } = req.params; // Mengambil ID dari parameter URL
